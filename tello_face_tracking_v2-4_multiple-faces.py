...
def track_face_in_video_feed(exit_event, show_video_conn, video_writer_conn, run_pid, track_face, fly=False,
    ...
    :param fly: Flag used to indicate whether the drone should fly.  False is useful when you just want to see the video stream.
    :type fly: bool
    ...
    tello = Tello("192.168.87.23")  # Add your Tello IP address
    ...
    while not exit_event.is_set():
        ...
        if run_pid:
            ...
            # Calculate the desired rotation speed based on the face position
            rotation_speed = int(pan_update * 0.7)

            # Check if the rotation speed is beyond a threshold to start rotating the drone
            if is_rotating and abs(rotation_speed) <= 10:
                rotation_counter += 1
                if rotation_counter >= 36:
                    is_rotating = False
                    rotation_counter = 0
                    tello.send_rc_control(0, 0, 0, 0)  # Stop rotation
            else:
                rotation_counter = 0

            if is_rotating:
                # Rotate the drone
                tello.send_rc_control(0, 0, 0, rotation_speed * rotation_direction)
            else:
                # Calculate the desired distance to move the drone in front of the face
                distance_to_move = int(distance_update * 0.3)  # Adjust the factor as needed

                if track_face and fly:
                    # left/right: -100/100, forward/backward: -100/100
                    tello.send_rc_control(int(pan_update // 3), distance_to_move - 100, int(tilt_update // 2), 0)
